...
@pytest.mark.parametrize("order_button_click", ["header", "body"])
def test_successful_order_flow(driver, user_data, order_button_click):
    ...
    assert driver.current_url == URLs.BASE_URL, "Переход по логотипу Самоката не на главную страницу"

    home_page.click_logo_yandex_open_dzen_page()
    assert driver.current_url == URLs.YANDEX_URL, "Переход по логотипу Яндекса не на страницу Дзена"
